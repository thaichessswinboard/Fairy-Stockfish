...
/// Mate with KX vs KX. This function is used to evaluate positions with
/// king and plenty of material vs a lone king. It simply gives the
/// attacking side a bonus for driving the defending king towards the edge
/// of the board, and for keeping the distance between the two kings small.
template<>
Value Endgame<KXKX>::operator()(const Position& pos) const {
  ...
  if (pos.count<ROOK>(strongSide) >=1
      || pos.count<SILVER>(strongSide) > 1
      ||(pos.count<SILVER>(strongSide)     && pos.count<KNIGHT>(strongSide))
      ||(pos.count<SILVER>(strongSide)     && (pos.count<PAWN>(strongSide)>=1))
      ||(pos.count<SILVER>(strongSide)     && (pos.count<FERS>(strongSide)>=1))
      ||(pos.count<KNIGHT>(strongSide)     && (pos.count<FERS>(strongSide)>1))
      ||((pos.count<KNIGHT>(strongSide)>1) && pos.count<FERS>(strongSide))
      ||((pos.count<ROOK>(strongSide)     && ((pos.count< FERS>(strongSide) && pos.count<KNIGHT>(strongSide)))  || ( pos.count<ROOK>(weakSide) && pos.count<FERS>(weakSide)))
     ||((pos.count<ROOK>(strongSide)     && ((pos.count< FERS>(strongSide)>1 )&& ( DarkSquares & pos.pieces(strongSide, FERS)+(~DarkSquares & pos.pieces(strongSide, FERS)  ||  pos.count<ROOK>(weakSide)) )))
     || ((pos.count<ROOK>(strongSide)     && ((pos.count< FERS>(strongSide)>1 ) && ( DarkSquares & pos.pieces(strongSide, FERS)+(~DarkSquares & pos.pieces(strongSide, FERS) || pos.count<ROOK>(weakSide)))))
      || (pos.count< FERS>(strongSide) >= 3 && (DarkSquares & pos.pieces(strongSide, FERS) >=2) +  (~DarkSquares & pos.pieces(strongSide, FERS) ==1)
       || (pos.count< FERS>(strongSide) >= 3) &&  (~DarkSquares & pos.pieces(strongSide, FERS)>=2) + (DarkSquares & pos.pieces(strongSide, FERS) ==1) )))))
